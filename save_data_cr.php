...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql = "INSERT INTO data_proyek_cr (
            id, timestamp, cr_id, project_id, cr_date,
            requester_id, requester_role, change_category, change_type, change_trigger,
            change_description, root_cause, document_reference, document_link, wbs_code,
            activity_name, critical_path_status, bim_object_id, bim_element_name, object_location,
            risk_id, risk_category, risk_score_at_cr, estimated_time_impact_days, estimated_cost_impact,
            quality_impact, risk_score_change, affected_successors, rework_potential, impact_summary,
            system_recommendation, alternative_actions, priority_level, cr_status, approval_level,
            approval_decision, approval_notes, approval_date, implementation_status, implementation_progress,
            evidence_file, actual_time_impact_days, convert_to_knowledge
        ) VALUES (
            ?, ?, ?, ?, ?,
            ?, ?, ?, ?, ?,
            ?, ?, ?, ?, ?,
